...
// Validates that input is a number and within the valid range
...
class TicTacToe {
private:
    char board[3][3];           // 3x3 game board storing 'X', 'O', or ' '
    char currentPlayer;         // Tracks whose turn it is ('X' or 'O')
    int moveCount;              // Counts total moves made (used for draw detection)
...
public:
    ...
    void displayBoard() {
        ...
        for (int row = 0; row < 3; row++) {
            ...
            for (int col = 0; col < 3; col++) {
                cout << board[row][col];
                if (col < 2) cout << " | ";  // Column separators
            }
            cout << "\n";
            if (row < 2) cout << "  -----------\n";  // Row separator
        }
    ...
    // Updates the board and increments move count
